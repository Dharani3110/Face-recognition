...
proc vTcl:copy {{w ""}} {
    # Cut/copy/paste handled by text widget only createCompound
    ...
    if {$class == "Toplevel"} {
        vTcl::MessageBox -type ok \
            -title "PAGE" \
                        -message "You cannot copy the top-level window."
        return
    }
...
proc vTcl:delete {recipient {w ""}} {
    ...
    if {$p_class == "TPanedwindow"} {
        ::vTcl::MessageBox -icon error -message \
            "You cannot cut or delete a pane from a paned window.\nSelect paned window and edit panes instead." \
            -title "Save Error!" -type ok
                return
    }
    if {$p_class == "TNotebook"} {
                ::vTcl::MessageBox -icon error -message \
...
proc vTcl:paste {{fromMouse ""} {w ""}} {
    ...
    # vTcl(mouse,y) don't seem to be set anywhere, hence my use if the
    # two x and y variables from above.
    # if {$fromMouse == "-mouse" && $mgr == "place"} {    #      set opts "-x $vTcl(paste,x) -y $vTcl(paste,y)"
    # } elseif {$mgr == "place"} {
    #      set opts "-x 0 -y 0"
    # }
    # Above replaced by Rozen 2/16/16
    set opts "-x $vTcl(paste,x) -y $vTcl(paste,y)"
    append opts " -width $vTcl(copy_width) -height $vTcl(copy_height)"

    set vTcl(paste,class) [vTcl::compounds::getClass clipboard scrap]

...
namespace eval ::vTcl::compounds {
    ...
    proc createCompound {target type compoundName \
        ...
        # Among other uses this generates an internal function
...
#append output "dpr target\n"
        ...
        if {$vTcl(copy)} {
            ...
            #append output "$vTcl(tab)\$top configure -menu \$top.$last_piece\n"
#append output "dmsg About to do the configure thing.\n"
            if {$class == "Menu"} {
                append output    "$vTcl(tab)\$top configure -menu \$target\n"
            } ;# end if
        }
#append output "vTcl:printaliases \"bottom of compoundCmd.\"\n"
        append output "\}\n\n"
...
#dpr output  ;# This prints out the code for the copy operation.
        return $output
    }
...
#"
    ...
    proc autoPlaceCompound {type compoundName gmgr gopt} {
        ...
        set rootclass [set ${spc}::class]
        if {$rootclass == "Toplevel"} {
            set namePrefix $rootclass ;# This looks really
                                       # wrong. should be 3 characters
                                       # of rootclass??? Rozen.
        } else {
            set namePrefix cpd
        }
        ...
        if {$rootclass == "Menu"} {set namePrefix "m"}

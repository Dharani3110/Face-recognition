...
proc {vTcl:prefs:data_exchange} {save_and_validate} {
    ...
    if {$save_and_validate} {
        ...
        if {[string index $prefs::gui_font_fixed 0] == "T"} {
            set vTcl(pr,gui_font_fixed) $prefs::gui_font_fixed
        } else {
        set vTcl(pr,gui_font_fixed) \
            [font configure $prefs::gui_font_fixed]  ;# Rozen 12/14/12
        }

        if {[string index $prefs::gui_font_text 0] == "T"} {
            set vTcl(pr,gui_font_text) $prefs::gui_font_text
        } else {
            set vTcl(pr,gui_font_text) \
                [font configure $prefs::gui_font_text]  ;# Rozen 12/14/12
        }

        if {[string index $prefs::gui_font_menu 0] == "T"} {
            set vTcl(pr,gui_font_menu) $prefs::gui_font_menu
        } else {
            set vTcl(pr,gui_font_menu) \
                [font configure $prefs::gui_font_menu]
        }
        if {[string index $prefs::gui_font_tooltip 0] == "T"} {
            set vTcl(pr,gui_font_tooltip) $prefs::gui_font_tooltip
        } else {
            set vTcl(pr,gui_font_tooltip) \
                [font configure $prefs::gui_font_tooltip]
        }
        set vTcl(actual_gui_font_tooltip_name) $prefs::gui_font_tooltip
        # So that I can start using the any new font specifications right away.
        set vTcl(actual_gui_font_tooltip_desc) \
            [vTcl:condence_font_description $prefs::gui_font_tooltip]
...
# GUI Menu Font
    ...
    button $button_frame.change \
        -text "Change ..." \
        -command "
                  vTcl:prefs:browse_font $prefs::gui_font_menu
                  $menu_font_label configure -font \$prefs::gui_font_menu
                  set vTcl(actual_gui_font_menu_name) \$prefs::gui_font_menu"
    pack configure $button_frame.change -side top
...
# GUI Tooltip Font
    ...
    button $button_frame.change \
        -text "Change ..." \
        -command "
                 vTcl:prefs:browse_font $prefs::gui_font_tooltip
                 $tooltip_font_label configure -font \$prefs::gui_font_tooltip"
#        -command "vTcl:prefs:browse_font $prefs::gui_font_tooltip
#                  $tooltip_font_label configure -font \$prefs::gui_font_menu"
    pack configure $button_frame.change -side top
...
#         set analog_colors [::colorDlg::analogous_colors  $::prefs::guibgcolor]
# dpr analog_colors
#         lassign $analog_colors ::prefs::guianalogcolor
#         #lassign $analog_colors ::prefs::guianalog_color_p \
#             ::prefs::guianalog_color_m
#     }
#     if {$variable == "::prefs::menubgcolor"} {
#         set ::prefs::guicomplement_color \
#             [::colorDlg::complement  $::prefs::guibgcolor]
#         set menu_analog_colors [::colorDlg::analogous_colors  \
#                                     $::prefs::menubgcolor]
...
proc vTcl:prefs:color_pref {w text variable} {
    ...
    # Rozen, end of default color button.
    # Add swatch for analog color.
    # set show [vTcl:formCompound:add $color_frame button \
    #             -relief raised \
    #             -text "show" -bg $::prefs::bganalogcolor -width 8]
    # pack configure $show -side right -padx 1 -pady 1
    # $show configure -command "vTcl:show_color $show"
}

# proc vTcl:show_color {visual} {
#     $visual configure -bg $::prefs::bganalogcolor
# }
...
proc {vTcl:prefs:bgcolor} {tab} {
    ...
    ##----------------------------------------------------------------
    # Following added by Rozen. 11/6/12       variable guifgcolor       $vTcl(pr,guifgcolor)

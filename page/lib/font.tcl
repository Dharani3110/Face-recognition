...
proc vTcl:font:prompt_user_font_2 {font_desc} {
    # Enter here when we want to select font.
    global vTcl
    if {$font_desc == ""} {
        set font_desc vTcl(pr,gui_font_dft)
    } ;# end if
...
proc vTcl:font:init_stock {} {
    ...
    set gtip [is_default_font gui_font_tooltip]
    set vTcl(font,gui_font_tooltip) $gtip
    set vTcl(default_gui_font_names) [list $gd $gt $gf $gm $gtip]
    ...
    set vTcl(actual_gui_font_tooltip_name) $gtip   ;# $vTcl(font,gui_font_menu)
    # These are descriptions like "-family times -size 12 -weight bold".
    # I don't think that I need this block of code.  10/13
    set vTcl(actual_gui_font_dft_desc) \
        [vTcl:condence_font_description $vTcl(pr,gui_font_dft)]
    set vTcl(actual_gui_font_text_desc) \
        [vTcl:condence_font_description $vTcl(pr,gui_font_text)]
    set vTcl(actual_gui_font_fixed_desc) \
        [vTcl:condence_font_description $vTcl(pr,gui_font_fixed)]
    set vTcl(actual_gui_font_menu_desc) \
        [vTcl:condence_font_description $vTcl(pr,gui_font_menu)]
    set vTcl(actual_gui_font_tooltip_desc) \
        [vTcl:condence_font_description $vTcl(pr,gui_font_tooltip)]
}
...
# This is now just a list of Tk fonts for reference.
...
proc vTcl:font:add_GUI_font {font_name font_descr} {
    # This is called when we load an existing GUI-tcl file. It gets rid
...
#     set font_names [lsort -unique $vTcl(default_gui_font_names)]
# dpr font_names    
#     foreach f  $font_names {
#         puts $fileID "vTcl:font:add_GUI_font $f \\"
# dpr f 
# dmsg look for Tk        [string first "Tk" $f]    

#         if {[string first "Tk" $f] == -1} {
# dmsg inside Tk test            
#             set font_conf [font configure $f]
#             set font_c [vTcl:condence_font_description  $f]
#             # puts $fileID \"[font configure $font_c]\"
#             puts $fileID \"$font_c\"
#         }   
#     }
    if {[string first "Tk" $vTcl(actual_gui_font_dft_desc)] == -1} { 
        puts $fileID \
            "set desc \"[vTcl:condence_font_description \
    ...
    # --------------
    if {[string first "Tk" $vTcl(actual_gui_font_text_desc)] == -1} { 
        puts $fileID \
            "set desc \"[vTcl:condence_font_description \
                            $vTcl(actual_gui_font_text_desc)]\""
        puts $fileID \
    ...
    # --------------
    if {[string first "Tk" $vTcl(actual_gui_font_fixed_desc)] == -1} { 
        puts $fileID \
            "set desc \"[vTcl:condence_font_description \
                            $vTcl(actual_gui_font_fixed_desc)]\""
        puts $fileID \
    ...
    # --------------
    if {[string first "Tk" $vTcl(actual_gui_font_menu_desc)] == -1} { 
        puts $fileID \
            "set desc \"[vTcl:condence_font_description \
    ...
    # --------------
    if {[string first "Tk" $vTcl(actual_gui_font_tooltip_desc)] == -1} { 
        puts $fileID \
            "set desc \"[vTcl:condence_font_description \
                            $vTcl(actual_gui_font_tooltip_desc)]\""
        puts $fileID \
...
proc vTcl:font:generate_font_user {fileID} {
    ...
    #puts $fileID {#############################################################################}
    #puts $fileID "####################  vTcl:font:generate_font_use Starting"
    ...
    foreach font $vTcl(dump,userFonts) {
        ...
        puts $fileID "    [vTcl:font:get_type $font] \\"
        #puts $fileID "    [vTcl:font:get_key $font]"
        puts $fileID "    vTcl:$font"              ;#  12/29/14
    }
    #puts $fileID "####################  vTcl:font:generate_font_use Ends"
}

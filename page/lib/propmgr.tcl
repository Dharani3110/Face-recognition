...
proc vTclWindow.vTcl.ae {args} {
    ...
    bind $w.ea <KeyRelease-Return> "
        ...
        incr vTcl(change); vTcl:show_wtree" ;# Line added 5/27/16 & 4/4/17. Rozen
    bind $w.ea <FocusOut> "
        ::vTcl::properties::setAlias \$::vTcl(w,widget) ::vTcl(w,alias) $w.ea
        incr vTcl(change); vTcl:show_wtree" ;# Line added 5/27/16 & 4/4/17. Rozen
    ...
    if {[info exists vTcl(show_variant)] && $vTcl(show_variant)} {
        ...
        bind $w.ea <KeyRelease-Return> "
          ::vTcl::properties::setAlias \$::vTcl(w,widget) ::vTcl(w,alias) $w.ea
          incr vTcl(change); vTcl:show_wtree"
        bind $w.ea <FocusOut> "
          ::vTcl::properties::setAlias \$::vTcl(w,widget) ::vTcl(w,alias) $w.ea
        in`cr vTcl(change); vTcl:show_wtree"
    }
    ...
    vTcl:setup_vTcl:bind $ae
    vTcl:BindHelp $ae PropManager

    bind $base.cpd21.03 <Enter> \
        {vTcl:bind_mousewheel  $::vTcl(gui,ae).cpd21.03}
    bind $base.cpd21.03 <Leave> \
        {vTcl:unbind_mousewheel $::vTcl(gui,ae).cpd21.03}
    ...
    global vTcl
        switch $option {
            ...
            -tearoffcommand {
                ...
                #     dmsg calling msg box
                #     ::vTcl::MessageBox -icon error  \
                #         -message "Syntax Error: Illegal name -\"$value\"" \
                #         -title "Syntax Error"
                #     update idletasks
                #     #return
                # }
...
# Rozen 12/30/17 Should I add:  vTcl:place_handles $target
    }
}
...
proc vTcl:prop:geom_config_mgr {target option variable value args} {
    # Destination when user modifies geometric option  Return.
    global vTcl
    #set mgr $args   ;# I only support mgr = place as seen below.
    vTcl:setup_unbind_widget $target

    if {$value == ""} {
        set value [set $variable]
    }
    # Since place figures many geometric properties as the sum of
    # an absolute and relative value and the Attribute Editor
    # shows both values as the complete value, if we change one
    # then a zero value should be passed to the place command.
    switch -exact $option {
        -relx {
    ...
    vTcl:update_widget_info $target         ;# New Sept 2018
...
proc vTcl:prop:update_attr {} {
    ...
    #catch {
        ...
        foreach i "$vTcl(m,$mgr,extlist) $vTcl(m,$mgr,list)" {
            if {$i == "title"} continue ;# Rozen moved title from geom section.
            # New stuff for handling prop,geom and dflf,origin.
    ...
    switch $type {
        ...
        combobox {
            ...
            button ${base}.b -image ellipses -width 12 -padx 0 -pady 1 \
                -command "
                   incr vTcl(change)
                   vTcl:prop:combo_edit ${base}.c $option"
        ...
        color {
            ...
            vTcl:entry ${base}.l -relief sunken  \
                ...
                -validate all -validatecommand {vTcl:incr_change} \
...
#            pack ${base}.f -side right -fill y -pady 1 -padx 1
        ...
        command {
            ...
            vTcl:entry ${base}.l -relief sunken  \
                ...
                -validate all -validatecommand {vTcl:incr_change} \
                -highlightthickness 1 -fg black  -insertborderwidth 3
                #-validate focusout -validatecommand {vTcl:isident %P %W} \
                #-fg white
# Removed by Rozen
#             button ${base}.f \
#                 -image ellipses  -width 12 \
...
#            pack ${base}.f -side right -fill y -pady 1 -padx 1
    ...
    ## geometry or widget options is currently provided.
    if {!$isGeomOpt} {
        bind $label <ButtonRelease-3> \
    "vTcl:propmgr:show_rightclick_menu $vTcl(gui,ae) $option $variable %X %x %Y"
...
proc vTcl:propmgr:show_rightclick_menu {base option variable X x Y} {
...
proc vTcl:prop:apply_opt {w opt varName extent action {user_param {}}} {
    global vTcl
    vTcl:destroy_handles          ;#  we need to destroy the handles.
    set class [vTcl:get_class $w]

...
proc vTcl:update_widget_info {target} {
    ...
    foreach i $vTcl(attr,winfo) {
        if {$i == "manager" && $target == "."} {
            ...
            set vTcl(w,$i) place
        } elseif { $i == "manager" && [winfo class $parent] == "TNotebook"} {
            set vTcl(w,$i) place
        } else {
...
proc vTcl:new_widget_name {class base} {
    ...
    # This just creates the name for the new widget.
    global vTcl
    ...
    if {$class != "Toplevel"} {
        ...
        switch $class {
            PNotebook {
                ...
                ttk::style layout PNotebook.Tab {
        ...
        if {$class == "TSizegrip"} {
            ...
            # # FIXME: put in orient stuff.
            # append do \
            #     "set orientation \[$new_widg cget -orient\]\n"
            # append do \
...
#WAS NOT DELETEING WIDGETS WITH SPECIAL DELETE COMMANDS SUCH AS TOPLEVEL
    ...
    set vTcl(widget_class,$new_widg) $class;# To tell Tnotebooks from PNotebooks.
    return $new_widg
}
...
proc vTcl:set_alias {target {alias ""} {noupdate ""}} {
    ...
    if {![lempty $alias] && $alias != $was} {
        ...
        # removed the  following to test vTcl(aliases) 8/31/14
...
proc vTcl:new_widget {autoplace class button {options ""}} {
    ...
    "
    ;# NEEDS WORK I am temporarily removing the following binding
    ;# because I want to use <Shift-Button-1> to select the containing
    ;# widget.
...
proc vTcl:widget:get_tree_label {w} {
    ...
    # alias, with the exception of the Toplevel widget where the class
    # name is calculated and returned returned. In essence I changed
    # the getTreeLabel routine in toplevel.wgt to calculate the class
    # name and copied the change here.
    global vTcl
    set c [vTcl:get_class $w]
    if {$c == "TNotebook" && [string first ".pN" $w] > -1} {
        set c "PNotebook"
    }
    if {$c == "Toplevel"} {
       set label  [wm title $w]
       if {$label == ""} {
          # return  alias
          set l $widget(rev,$w)
      } else {
          # Replace blanks with underscore to get the class name.
          set l [string map { " " _ } $label ]
      }
       return "$c: $l"
    }

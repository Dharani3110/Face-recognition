...
proc vTcl:wm:enable_geom {} {
    ...
    array set rev_state {1 disabled 0 normal} ;# Added by Rozen 1/31/15
    if {$vTcl(w,class) == "Menu"} {
        set origin_state disabled
        set size_state   disabled
    } else {
        set origin_state $rev_state($vTcl(w,wm,dflt,origin))
...
proc vTcl:wm:relative_geometry {target var value} {
    # Rozen 1/4/15 This proc called only if the corresponding value is
    # changed in the atribute editor.
    global vTcl
    incr vTcl(change)yes

    set ::widgets::${target}::prop,geom $value
}

proc vTcl:place:conf_geom {target var value} {
    # Entered when one changes the Boolean attribute "locked".
    # This spec setting this routine as the callee associated with the
    # Boolean attribute occurs in global.tcl with the setting of
    # vTcl(m,place,locked).
    global vTcl
    incr vTcl(change)
    set ::widgets::${target}::locked  $value
    set vTcl(${target},locked) $value
    ;# NEEDS WORK this isn't doing any good but we do need to save it.
    vTcl:prop:save_opt_value $target "locked" $value
    vTcl:init_wtree
}

proc vTcl:wm:conf_geom {target var value} {
    # This proc called only if the corresponding value is
    # changed in the atribute editor.
    global vTcl
    incr vTcl(change)
    set ::widgets::${target}::dflt,origin  $vTcl(w,wm,dflt,origin) ;#$value
...
proc vTcl:wm:dump_info {target basename} {
    ...
    foreach wm_option $vTcl(m,wm,savelist) {
        if {[info exists ::widgets::${target}::${wm_option}]} {
            ...
            append out \
               "set $wm_option [vTcl:at ::widgets::${target}::${wm_option}]\n"
        }
    }
    append out "$vTcl(tab)\}\n"
    return $out
...
proc vTcl:edit_target_menu {target} {
    ...
    if {$menu == ""} {
        # It's a new menu. So we get a name for it, create it, and
    ...
    # Now we go edit it by opening the menu editor
...
proc vTcl:core:set_option {target option description} {
    ...
    # cancelled?
    if {$newvalue == "*"} {
        # Rozen changed this to be an illegal character in a python
...
proc vTclWindow.vTcl.itemEdit {base} {
    ...
    # DEFINING ALIASES
    ###################
    # ItemsListbox alias for listbox containing list of tabs or pages. Rozen
    vTcl:DefineAlias $base.cpd37.01.cpd38.01 ItemsListbox vTcl:WidgetProc $base 1
    # PropertiesFrame contains all the properties.
    ...
    # Button but39 is the OK button. Rozen
    ::vTcl::CancelButton   $base.fra34.but39  \
    ...
    # Rozen the list box where the pages are listed.  This is the left window
    listbox $base.cpd37.01.cpd38.01 \
        ...
        -background #ffffff
    set vTcl(edit,left_listbox) $base.cpd37.01.cpd38.01
  ...
  # Rozen to aid in debugging
    ...
    # This is the right hand window containing the attributes.
    canvas $base.cpd37.02.sw.c \
        -highlightthickness 0 -yscrollincrement 20
    set vTcl(edit,right_canvas) $base.cpd37.02.sw.c
    ###################
    # SETTING GEOMETRY
    ###################
    pack $base.fra34 \
...
#        -in $base.cpd37.02.sw
    ...
    bind $vTcl(edit,right_canvas) <Enter> \
        {vTcl:bind_mousewheel  $::vTcl(edit,right_canvas)}
    bind $vTcl(edit,right_canvas) <Leave> \
        {vTcl:unbind_mousewheel $::vTcl(edit,right_canvas)}

    bind $vTcl(edit,left_listbox) <Enter> \
        {vTcl:bind_mousewheel  $::vTcl(edit,left_listbox)}
    bind $vTcl(edit,left_listbox) <Leave> \
        {vTcl:unbind_mousewheel $::vTcl(edit,left_listbox)}
...
# Because I didn't know where better to put this function. It is part of the edit tab stuff.
proc vTcl:fudge_compound_attribute {suffix} {
}
...
namespace eval ::vTcl::itemEdit {
    ...
    proc edit {target cmds} {
        ...
        variable suffix
        variable class
        incr counter                    ;# NEEDS WORK - do I want this line???
...
# dpr class
# if {$option == "-image"
#     && [string first ".pN" $target]} {
#         continue
# }
                set default [lindex $opt 3]
                # Rozen. Inserted because tab configuration list have
                # 2 items not 4.
                if {[llength $opt] == 4} {
                ...
                # Rozen. For notebooks I want initial value of
                # compound to left. 12/31/17
                # if {$option == "-compound"} {
                #     if {[string first ".pNo" $top_w]} {
                #         if {$value == "none"} {
                #             set value "left"
                #         }
                #     }
    ...
    proc keyRelease {top option var boxvar} {
        ...
        variable current
        variable target
        set conf [::$cmds($top)::itemConfigure $target($top) \
                      $current($top) $option]
    ...
    proc init {top w cmdsEdit} {
        ...
        variable current
        variable adding
        variable top_w           ;# Added by Rozen for handling PNotebook.
        set top_w $w
        ...
        initBoxes $top
        initProperties $top   ;# initProperties at about line 800
        selectItem $top $current($top) ;# selectItem at about line 860
        wm title $top [::$cmds($top)::getTitle $w]

        ::vTcl::notify::subscribe delete_widget $top \
            ::vTcl::itemEdit::widgetDeleted
        ::vTcl::notify::subscribe deleted_childsite $top \
            ::vTcl::itemEdit::childsiteDeleted
    }
    ...
    proc initProperties {top} {
        ...
        foreach option $options {
            if {$option == "-image" && [string first ".pNo" $top_w] > -1} {
                continue}
            if {$option == "-compound"  && [string first ".pNo" $top_w] > -1} {
                continue}
...
#after 5000 {vTcl:exit}
    ...
    proc selectItem {top index} {
        ...
        if {$index == ""} {
            # Hate to think that this is a fix.
            return
        } ;# end if
        ...
        set current($top) $index ;# The page selected.
    ...
    proc close {top} {
        # Routine called when the OK check is selected.
        variable cmds
        variable target
        variable suffix
        variable current

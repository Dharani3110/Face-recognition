...
proc vTcl:set_tooltip {target} {
    # Routine call for a window in which user can add a multiple line
    # tool tip for the target widget.
    global vTcl
    set base .vTcl.[vTcl:rename $target]
    vTcl:text_window $base "Set Tooltip" $target 1
    tkwait window $base
    # They closed it without hitting the done button.  Just forget it.
    if {![info exists vTcl(x,$base)]} { return }
    # Causes tooltip text to appear in the Attributer Editor.
    set vTcl(w,opt,-_tooltip) $vTcl(x,$base) ;
    # causes the text to be save so that Python generation can find it.
    vTcl:prop:spec_config_cmd $target -_tooltip vTcl(w,opt,-_tooltip)\
        $vTcl(x,$base) vTcl:config_balloon
    unset vTcl(x,$base)
    vTcl:create_handles $target
}
...
proc vTcl:text_window {base title target {tooltip 0}} {
    global vTcl
    ...
    bind $base <Key-Escape> "$base.butfr.but53 invoke" ;# invokes cancel button
    ...
    if {$tooltip} {
        $base.cpd48.03 insert end $vTcl(w,opt,-_tooltip)
    } else {

...
# vTclWindow.vTcl.bind            Main bind editor window.
# vTclWindow.vTcl.newbind         The "Advanced" window.
# ::widgets_bindings::add_binding
# vTclWindow.vTcl.newtag          Window for Add->Tag..
# ::widgets_bindings::fill_bindings
# ::widgets_bindings::listbox_click
# ::widgets_bindings::is_editable_tag
...
proc vTcl:get_bind {target} {
    ...
    if {[winfo exists .vTcl.bind]} {
        ...
        wm title [BindingsEditor] "Widget bindings for $name"
    }
}
...
proc vTclWindow.vTcl.bind {args} {
    ...
    foreach ev {Button-1        Button-2        Button-3
        ...
        $base.m37.men38 add command \
            ...
            -label $ev \
            -foreground $vTcl(actual_fg)   ;# Rozen 2013
    }
    $base.m37.men38 add command \
    ...
    ::vTcl::OkButton $base.fra22.but34 \
        -command {
            if { [::widgets_bindings::save_current_binding] != 1 } {
                        Window destroy .vTcl.bind
            }
            incr vTcl(change)
         }
    vTcl:set_balloon $base.fra22.but34 "Save and Close"
    ...
    # Stuff above has to do with menus and buttons at the top of window.
    ...
    # This is a popup menu for the blue entries at the top of left pain.
    ...
    #         -command "TextBindings insert insert %%Y"
    #     # Since this popup doesn't seem to make sense in the PAGE
    #     # context because all that is allowed is a lambda, commenting
    #     # following statement will prevent the menu from popping up. 6/10/18
    ...
    # Following causes the editor window geometry to be set from saved window
    catch {wm geometry $base $vTcl(geometry,$base)}
    wm deiconify $base
}

proc vTclWindow.vTcl.newbind {base} {
    # This is the window opened by the selection of "Advanced".
    global vTcl
    if {$base == ""} {
    ...
    label $base.fra20.lab21 \
        -borderwidth 1 -foreground $vTcl(actual_fg) \
    ...
    listbox $base.fra23.cpd35.01  ;#This is the left hand listbox in
                                   #the advanced window. Rozen
    ...
    set bindingsEventEntry ""  ;# Rozen hack to clear event entry on start up.
    ...
    button $base.fra37.but01 -text "Add" -width 8 \
        -command {
            if {$bindingsEventEntry != ""} {
                 ...
                 set bindingsEventEntry "" ;# Rozen kludge to clear
                                            # entry on next
                                            # Advanced... selection
             }
        }
    ...
    button $base.fra37.but02 -text "Cancel" -width 8 \
        -command {BindingsInsert hide
         set bindingsEventEntry ""          ;# Rozen kludge -see above.
        }
    ...
    # Add virtual events to the left side of window.
    set ve [event info]
    #lappend ve "<<ListboxSelect>>"
    # [event info] lists of all virtual events curently defined.
    foreach event [lsort $ve] {
        if {[string match <<TkCon*>> $event]} { continue }
        BindingsEvents insert end $event
    }
    # foreach event [lsort [event info]] {
    #     if {[string match <<TkCon*>> $event]} { continue }
    #     BindingsEvents insert end $event
    # }
    # Rozen. Pick up the virtual events defined in the wgt page.
    ...
    bind $top.fra23.cpd35.01 <Enter> \
        {vTcl:bind_mousewheel  $::vTcl(gui,newbind).fra23.cpd35.01}
    bind $top.fra23.cpd35.01 <Leave> \
        {vTcl:unbind_mousewheel $::vTcl(gui,newbind).fra23.cpd35.01}
...
proc vTclWindow.vTcl.newtag {base} {
    ...
    wm focusmodel $base passive
    wm geometry $base 340x350+149+138
    wm withdraw $base
    ...
    # Looks like the list of bindings. Rozen
    listbox $base.fra24.cpd26.01     ;# ListboxTags widget
...
namespace eval ::widgets_bindings {
    ...
    proc {::widgets_bindings::listbox_click} {} {
        ...
        set widgets_bindings::lastselected [lindex \
                                 [ListboxBindings curselection] 0]
        ::widgets_bindings::enable_toolbar_buttons
    ...
    proc {::widgets_bindings::is_editable_tag} {tag} {
        ...
        if {[lsearch -exact \
                 [::widgets_bindings::get_subwidgets $target] $tag] >= 0} {
            return 1
        }
        return 0
    }
    ...
    proc {::widgets_bindings::add_binding} {event} {
        ...
        set index 0       ;# Rozen's hack for first entry.

...
# Copyright (C) 2013-2019 Donald Rozenberg
#
#
...
proc vTcl:fonts_actually_used {{selfie ""}} {
    ...
    foreach f [lsort -unique $vTcl(fonts_used)] {
        ...
        #     lappend putout $f
        #}
        #set font_cfg [vTcl:prune_font_string $font_cfg]
        set font_cfg [vTcl:condence_font_description $font_cfg]
        append vTcl(toplevel_config) \
...
proc vTcl:python_configure_special_opts {widget widname} {
    ...
    if {[info exists options(-_tooltip)]} {
      ...
      "$vTcl(tab2)ToolTip(self.$widname, \"$vTcl(actual_gui_font_tooltip_desc)\", '''$msg''', delay=0.5)\n"
            ...
            append vTcl(py_initfunc) \
           "$vTcl(tab2)tooltip_font = \"$vTcl(actual_gui_font_tooltip_desc)\"\n"
            append vTcl(py_initfunc) \
      "$vTcl(tab2)ToolTip(self.$widname, tooltip_font, '''$msg''', delay=0.5)\n"
...
proc vTcl:python_source_widget {target prefix parentframe} {
    ...
    if {$widclass == "TNotebook"} {
        ...
        Scale {
            ...
            append vTcl(py_initfunc) \
          "$vTcl(tab2)self.$widname = tk.Scale($pframe, from_=$from, to=$to)\n"
            #vTcl:relative_placement
...
proc vTcl:python_menu_font {font_i} {
 ...
 #-family Purisa -size 14 -weight normal -slant roman -underline 0 -overstrike 0
    ...
    if {$font_i in $vTcl(standard_fonts)} {
        return "font=\"$font_i\""
    }
    if {[string index $font_i 0] == "\{"} {
        foreach value $font_i {
        }
        return "font=\"$font_i\""
    }
    if {[string first "-family" $font_i] == -1} {

    # I think that we have a font name.
        lappend vTcl(fonts_used) $font_i
        #return "font=self.$font"
        #return "font=$font"         ;# 2/9/19
        set font_i [font actual $font_i]
    }
    if {$font_i == "" } {
        # I don't think that I hit this batch of code.
        set font_i vTcl(pr,gui_font_menu)
        #set font TkMenuFont
    }
    if {$font_i != ""} {
        set font_string "font=("
        # Reformat font sting for python
        append font_string [vTcl:prune_font_string $font_i] ")"
    }
    return $font_string
}

proc vTcl:prune_font_string {font_i} {
    # Reformat font sting for python
    foreach {prop value} $font_i {
        switch $prop {
            -family {
...
$vTcl(tab)print ('prog_call = {}'.format(prog_call))
$vTcl(tab)prog_location = os.path.split(prog_call)\[0\]
$vTcl(tab)print ('prog_location = {}'.format(prog_location))
$vTcl(tab)sys.stdout.flush()\n"    ;# if removed save the \n"
    }
...
proc vTcl:calc_relative_geometry {target} {
    ...
    # Split the target geometry into w, h, x, y
    foreach {w h x y} [split [winfo geometry $target] "x+"] {}
    #set x [expr $x - [expr $x % 5]] ;# NEEDS WORK I must need this stuff.
    #set y [expr $y - [expr $y % 5]] ;# 2/17
    set parent_window [winfo parent $target]
    ### Have to play games with the TNotebook because only one of the
    ### children widgets is mapped and the geometry of unmapped widget
    ### is unusable. So we go to the grand parent and run thru the

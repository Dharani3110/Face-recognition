...
proc vTcl:get_class {target {lower 0}} {
    ...
    if {$class == "TNotebook"} {
        # Since target could be something like .top37.pNo38.t0.tNo39
        # we have to look at the last piece.
        set pieces [split $target "."]
        set last_piece [lrange $pieces end end]
        if {[string first "pNo" $last_piece] > -1} {
            set class "PNotebook"
        }
    }
...
#         }
        ...
        switch $class {
            ...
            TNotebook {
               ...
               #  append result "$vTcl(tab)ttk::style layout $class.Tab {
    ...
    if {$class == "PNotebook"} {
        ...
        #append result "$vTcl(tab)$target configure -style \"PC.TNotebook\"\n"
            append result \
                ...
                #"$vTcl(tab)\$top.$l_p configure -style \"PC.TNotebook\"\n"
        }
    }
...
proc vTcl:dump:widgets {target} {
    # Run thru the widget tree duping as we go.
    global vTcl classes
    set output ""
...
## These are commands we want executed when we re-source the project. Actually occurs at the bottom rather than near top.
...
"set btop \"\"
if {\$vTcl(borrow)} {
$vTcl(tab)set btop .bor\[expr int(\[expr rand() * 100\])\]
$vTcl(tab)while {\[lsearch \$btop \$vTcl(tops)\] != -1} {
$vTcl(tab)$vTcl(tab)set btop .bor\[expr int(\[expr rand() * 100\])\]
$vTcl(tab)}
}
set vTcl(btop) \$btop
Window show .
Window show $i \$btop
if {\$vTcl(borrow)} {
$vTcl(tab)\$btop configure -background plum
}
"
    return $string
}
        ...
        foreach resource {image font} {
            ...
            foreach type {stock user} {
                foreach object [vTcl:at used_${resource}] {
                    if {[lsearch [vTcl:at vTcl(${resource}s,$type)] \
                             $object] > -1} {

...
namespace eval ::menu_edit {
    ...
    proc {::menu_edit::fill_command} {top command} {
        # Rozen. Recursive function which fills in the menu.
        global widget
    ...
    proc {::menu_edit::fill_menu_list} {top m} {
        # This fills in the left side of the menu editor
        global widget
        ...
        switch $type {
            "cascade" {
                ...
                $menu add $type -label "NewCascade" -menu $nmenu \
                      ...
                      -backgroun $vTcl(actual_gui_menu_bg) \
                    -font $vTcl(actual_gui_font_menu_name) -compound left
                vTcl:init_wtree
    ...
    proc configCmd {top option variable} {
        ...
        # field! For now I am making the command a simple constant so
    ...
    proc {::menu_edit::update_current_a} {top} {
        # Rozen. Put this on so that it will only be called only when
        # I exit the menu editor.  That way I can save the size of the
        # editor window as a preference.
        global vTcl
        ::vTcl::ui::attributes::setPending  ;# Bottom of misc.tcl.
    ...
    proc {::menu_edit::click_listbox} {top} {
        ...
        # i.e. select a component in the listbox. Rozen
        global vTcl
...
proc vTclWindow.vTclMenuEdit {base menu} {
    ...
    # Button but32 is the OK button with the check.  Rozen
    ::vTcl::OkButton $base.fra21.but32 \
        -command \
         "::menu_edit::update_current_a $base; destroy $base; incr vTcl(change)"

...
proc vTcl:boolean_radio {path args} {
    ...
    global vTcl
# set wid $vTcl(w,widget)
# set     class [vTcl:get_class $wid]
# if {$var == "vTcl(w,place,locked)"  || $class == "Entry"} {
#     dmsg &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
# dargs
# dpr vTcl(w,place,locked)
# dpr var com
# dmsg var $var _$var
# dmsg \$var  = [set $var]
# #    set vTcl(w,place,locked) 0
# dmsg config cmd     $path configure -variable "_$var" -command "set $var \$_$var; $com"
# dmsg configure nitial value: vTcl:boolean_radio_get $var _$var
# }
...
proc vTcl:clean_pairs {list {indent 8} } {
    ...
    foreach i $list {
        ...
        if {$last == "-font"} {
            set i [vTcl:condence_font_description $i]
                   #vTcl:condence_font_description
        }
...
proc vTcl:set_timestamp { } {
    global vTcl
    set vTcl(timestamp) [clock format [clock seconds] -format {%b %d, %Y %I:%M:%S %p %Z}]
    return
}
...
proc vTcl:isident {string  {widget ""}} {
    # Test if string is an identifier provided that the preference is set.
    global vTcl
    if {!$vTcl(pr,check_identifiers)} {
        return 1
    }
    ...
    if {$string == ""} {return 1}
    # An identifier in Python 2 is ^[a-zA-Z_][a-zA-Z_0-9]*$
    set result [regexp {^[a-zA-Z_][a-zA-Z_0-9]*$} $string]

    # I decided not to use at this time - 12/4/17.
    # if {$widget != ""} {
    #     if {$result} {
    #         # OK
    ...
    # In Python 3 non-ASCII characters are allowed. The closest that I
    # can test for that in Tcl is to use the regular expression
    # ^[[:alpha:]_][[:alnum:]_]*$
    #set result [regexp {^[[:alpha:]_][[:alnum:]_]*$} $string]
    return $result
}
...
#${base} clearvalue
            ...
            command {
                ...
                vTcl:entry ${base}.l -relief sunken  \
                    -textvariable $variable -width 8 \
                    -highlightthickness 1 -fg black
                # Rozen. decided to get rid of the command button. 1/20/17
...
proc vTcl:condence_font_description {font_in} {

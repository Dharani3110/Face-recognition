...
def sharpening(image):
    """
    It is to apply image processing technique i.e sharpening to the image.
...
def L2_distance(face_encoding):
    ...
    for i in range(100):
        for index, train_image_encoding in enumerate(train_image_encodings):
            ref = train_image_encoding[0]
            distance = np.linalg.norm(face_encoding[0] - ref)
            if distance < min_distance:
                min_distance = distance
                name = train_image_encoding[1]
                (fldr_name, img_name) = name.split("/")
    return fldr_name


def detect_and_track_largest_face():
    """
    It is to detect and track the largest face detected.
    :return: It will return the image with bounding box around the largest detected face.
    """
